// smart_inventory_with_auto_donate.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
// --- NEW: Automatic donation for items nearing expiry ---
// This function will automatically create donation records for items whose expiry
// is within AUTO_DONATE_DAYS (including 0 = today). It will set the donated
// quantity equal to the current quantity and set item->quantity to 0.
// Minimal disturbance: only donation logic added; rest of program unchanged.

#define AUTO_DONATE_DAYS 3

void autoDonateNearExpiry(Item *inventory) {
    Item *temp = inventory;
    int autoCount = 0;
    while (temp) {
        if (temp->quantity > 0) {
            int days = daysUntilExpiry(temp->expiry);
            if (days <= AUTO_DONATE_DAYS && days >= 0) {
                // prepare donation record
                DonationRecord rec;
                rec.itemId = temp->id;
                strncpy(rec.itemName, temp->name, MAX_NAME-1);
                rec.itemName[MAX_NAME-1] = '\0';
                // Auto donor marker
                strncpy(rec.donorName, "AUTO_DONATION", MAX_NAME-1);
                rec.donorName[MAX_NAME-1] = '\0';
                strncpy(rec.donorContact, "SYSTEM", MAX_NAME-1);
                rec.donorContact[MAX_NAME-1] = '\0';
                rec.quantityDonated = temp->quantity; // donate all remaining qty
                getCurrentDate(rec.date, sizeof(rec.date));
                rec.next = NULL;

                // save donation record
                saveDonationRecord(&rec);
                logAction("AUTO_DONATE", rec.itemId, rec.itemName);

                // zero out inventory quantity (moved out for donation)
                char buf[200];
                snprintf(buf, sizeof(buf), "Auto-donated %d of %s", rec.quantityDonated, rec.itemName);
                typewrite(GREEN, TYPE_DELAY_MS);
                typewrite("\n", TYPE_DELAY_MS);
                printf(GREEN "[AUTO DONATE] %s (ID:%d) → Qty: %d | Expiry: %s\n" RESET,
                       rec.itemName, rec.itemId, rec.quantityDonated, temp->expiry);
    ...
    if (autoCount > 0) {
        printf(YELLOW "\nAuto-donation completed for %d item(s). Records saved to %s\n" RESET, autoCount, DONATION_FILE);
    }
}
...
int main() {
    ...
    typewrite("\nStarting Smart Inventory System (with Auto-Donate)...\n", TYPE_DELAY_MS);
    ...
    int choice;
    while (1) {
        // NEW: automatic donation check executed on each loop iteration
        autoDonateNearExpiry(inventory);

        typewrite("\n" CYAN "=== Smart Inventory Menu ===" RESET "\n", TYPE_DELAY_MS);
